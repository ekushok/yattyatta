...
public class ChartCreate {
	...
	// 棒グラフ
	AFreeChart getBarThisMonth(int month, int year) {
		// グラフにするデータの作成
		ArrayList<String> data = store
				.loadAll("select _id,timestring,price from log2");
		...
		for (int i = 0; i < 31; i++) {
			if (priceOnDay[i] != 0) {
				dataset.addValue(priceOnDay[i], "今月", String.valueOf(i + 1)
						+ "日");
			}
		}
		// グラフの生成
		AFreeChart chart = ChartFactory.createBarChart((month + 1) + "月の出費",
				"日付[日]", "金額[円]", dataset, PlotOrientation.VERTICAL, false,
				false, false);
	...
	AFreeChart getLineThisMonth(int month, int year) {
		...
		// グラフの生成
		AFreeChart chart = ChartFactory.createLineChart((month + 1) + "月の出費",
				"日付[日]", "金額[円]", dataset, PlotOrientation.VERTICAL, false,
				false, false);
